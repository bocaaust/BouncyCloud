...
function setup()
 ...
 --   sound(DATA, "ZgBAZgAhdWFGLjRS/wv+va2h1j405G0+XQAOeDZABTg9WTN6")
   ...
   lr = -1
    ...
    counterint = 240
    score = 0
    bound = physics.body(EDGE,vec2(-1000,0),vec2(WIDTH+1000,0))
    bound.sensor = true
    bound.type = STATIC
    ballg = false
...
function createblock()
   -- if lr then
    ...
    blocks[#blocks].linearVelocity = vec2(0,fallspeed)
...
--while x <= #(intarr) do
   ...
   -- end
    ...
    if counter == counterint then
        ...
        if ball.y > HEIGHT+ball.radius-50*xw then
            ball.y = HEIGHT+ball.radius-50*xw
    ...
    -- Do your drawing here
      ...
      --  tint()
        ...
        sprite("Project:block",WIDTH/2,0,WIDTH)
    end
